...
import pageHtml from '../data/web_hosting_html.json';
import pageScripts from '../data/web_hosting_scripts.json';
...
export default function WebHostingPageClient() {
  useEffect(() => {
    // Run independently of jQuery — this fix doesn't need it.
    if (!(window as any).__whnz_web_hosting_TabsInit) {
      (window as any).__whnz_web_hosting_TabsInit = true;
    ...
    // up manually.
    if (!(window as any).__whnz_web_hosting_PricingInit) {
      (window as any).__whnz_web_hosting_PricingInit = true;
    ...
    const tryRunScripts = () => {
      if (!jqLoaded) return;
      if ((window as any).__whnz_web_hosting_ScriptsRan) return;
      (window as any).__whnz_web_hosting_ScriptsRan = true;
      try {
