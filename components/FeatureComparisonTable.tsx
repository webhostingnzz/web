export default function FeatureComparisonTable({
  heading, sub, planNames, rows,
}: { heading: string; sub?: string; planNames: string[]; rows: { feature: string; values: string[] }[] }) {
  return (
    <section className="py-16 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="font-display font-extrabold text-3xl sm:text-[38px] mb-4">{heading}</h2>
          {sub && <p className="text-black/70 max-w-xl mx-auto">{sub}</p>}
        </div>
